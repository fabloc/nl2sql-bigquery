    ...
    for table_name in tables_list:
        ...
        if os.path.exists(samples_filename_path):
            with open(samples_filename_path) as stream:
                try:
                    # Read query sample file and replace placeholders for project_id, dataset_id, etc. using Jinja
                    data = {
                        'project_id': cfg.project_id,
                        'dataset_id': cfg.dataset_id
                    }
                    queries_samples_tmpl = stream.read()
                    j2_template = Template(queries_samples_tmpl)
                    queries_samples_str = j2_template.render(data)
                    # extracting yaml properties
                    table_queries_samples = yaml.safe_load(queries_samples_str)
