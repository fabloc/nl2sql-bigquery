...
data = {
    ...
    'events_start_date': date.today() - timedelta(days = 365),
...
events_schema_path = base_path / "events_schema.json"
...
uri = "gs://genai-cdp-demo/small_users.csv"
...
# To load a schema file use the schema_from_json method.
events_schema = client.schema_from_json(events_schema_path)

events_table = bigquery.Table(project_id + '.' + dataset_id + '.events', schema=events_schema)
events_table.description = "Table listing all the events generated by the users. Events are partitioned by session_created_at field with a daily granularity. Each event corresponds to a user session on the retailer website. It lists all the actions performed on the website, like navigation, purchasing, adding to the cart, etc."
events_table.partitioning = bigquery.TimePartitioning(field="session_created_at")
events_table.partitioning_type = bigquery.TimePartitioningType.DAY
table = client.create_table(events_table)
